      ...
      if (nrow(table)>=50 & (cty == "COOK" | cty == "DUPAGE")) { #Use only the weeks for which there are at least 50 obs
        for (p in ((5:9)*10)) {
          ...
          for (k in 1:100) {
            ...
            pb = pooledBin(table[extraction,"ISPOS"],table[extraction,"POOLSIZE"],pt.method = "bc-mle",scale = 1000)
            MIR = pb$p 
            MIRL = pb$lcl 
            MIRU = pb$ucl             
            error = (MIR-MIR0)/MIR0
...
write.csv(resultsC, file = "RandomMIRSamplingCountywithVarianceMLE.csv", row.names=FALSE)
save(resultsC, file = "RandomMIRSamplingCountywithVarianceMLE.Rdata")
